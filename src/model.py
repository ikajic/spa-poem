...
def create_model(D=512, query=False):
    ...
    input_episodic = utils.add_vocabularies(
            spa_voc, 'sensory', 'conceptualization')
    input_affect = utils.add_vocabularies(
            spa_voc, 'episodic', 'interoception')
    ...
    with spa.SPA('POEM') as model:
        ...
        model._episodic = Memory(
                input_vocab=input_episodic,
        ...
        ep_output_vectors = np.array(
                [spa_voc['episodic'].parse(key).v for key in
        ...
        # Conceptualization
        model._conceptualization = Memory(
                input_vocab=spa_voc['language'],
        ...
        nengo.Connection(
                model._language.output, model._conceptualization.input,
                transform=trans_mat.T, synapse=0.01)
        nengo.Connection(
                model._conceptualization.output, model._episodic.input,
                synapse=0.3)
        ...
        model._affect = Memory(
                input_vocab=input_affect,
...
model, _ = create_model(D=512, query=False)
